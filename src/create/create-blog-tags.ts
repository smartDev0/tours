...
import { AllTagWithPostQueryResult } from "../app-types/blog";
...
export const createBlogTags = async (
  ...
  const result = await graphql<AllTagWithPostQueryResult>(`
    query {
      allTagWithPostResult: allWpTag {
        nodes {
          name
          description
          slug
  ...
  result.data &&
    result.data.allTagWithPostResult.nodes.forEach((node) => {
      ...
      Array.from({ length: numberOfPages }).forEach((_, i) => {
        ...
        actions.createPage({
          path: currentPath,
          component: path.resolve("./src/templates/blog/tag/index.tsx"),
          context: {
            pagination: {
              olderPath: olderPath,
              newerPath: newerPath,
