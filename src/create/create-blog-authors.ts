...
import { AllAuthorWithPostQueryResult } from "../app-types/blog";
import { POSTS_PER_PAGE } from "../constants/blog";
import { getBlogPath, getBlogWithPagePath } from "../utils/path";

export const createBlogAuthors = async (
  actions: Actions,
  graphql: CreatePagesArgs["graphql"]
): Promise<void> => {
  const result = await graphql<AllAuthorWithPostQueryResult>(`
    query {
      allAuthorWithPostResult: allWpUser {
        nodes {
          name
          uri
          description
  ...
  result.data &&
    result.data.allAuthorWithPostResult.nodes.forEach((node) => {
      ...
      Array.from({ length: numberOfPages }).forEach((_, i) => {
        ...
        actions.createPage({
          path: currentPath,
          component: path.resolve("./src/templates/blog/author/index.tsx"),
          context: {
            pagination: {
              olderPath: olderPath,
              newerPath: newerPath,
